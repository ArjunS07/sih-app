...
Future<Account?> createAccount(String email, String password, String firstName, String lastName) async {
    ...
    Map<String, dynamic> body = json.decode(await response.stream.bytesToString());
...
Future<Student?> createStudent(Account account, String city, List<String> languages, School school, String board, String grade) async {
  ...
  http.StreamedResponse studentCreationResponse = await studentCreationRequest.send();
    Map<String, dynamic> body = json.decode(await studentCreationResponse.stream.bytesToString());
  ...
  final Uri joinSchoolUri = Uri.parse('$ROOT_URL/joinschool');
  var joinSchoolRequest = http.Request('POST', joinSchoolUri);

  var joinSchoolheaders = {
      'Content-Type': 'application/x-www-form-urlencoded'
  };
